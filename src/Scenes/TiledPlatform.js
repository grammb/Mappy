class TiledPlatform extends Phaser.Scene {
    ...
    create() {
        ...
        //creating array of sprites, each with matching name from Tiled file
        ...
        //now we need to tell ArcadePhysics about the power up sprites!
        this.physics.world.enable(this.bluePowerUps, Phaser.Physics.Arcade.STATIC_BODY);  //this line makes the sprites static Arcade Physics bodies
        this.bluePowerUpGroup = this.add.group(this.bluePowerUps) // adds the sprites into collision group, and now can be defined with other groups


         //JUICE IT >:) PARTICLES HERE 
         this.bluePowerUpGroupVfxManager = this.add.particles('kenney_sheet', 215);

         this.powerUpVfxEffect = this.bluePowerUpGroupVfxManager.createEmitter({
            follow: this.p1,
            quantity: 20,
            scale: {start: 1.0, end: 0.0}, //start big end small
            speed: {min: 50, max: 100}, //speed up thru lifespan
            lifespan: 800, //short lifespan
            on: false //do not immediately start, will trigger on collision
         });


        //now we define collision between player and power up
        this.physics.add.overlap(this.p1, this.bluePowerUpGroup, (obj1, obj2) => {
            this.powerUpVfxEffect.explode(); //trigger particle system
            obj2.destroy();    //remove the powerup
        });
        //  ^ from here, the power up is destroyed upon player collision ^ HAVE ADDED IN PARTICLE SYSTEM 
